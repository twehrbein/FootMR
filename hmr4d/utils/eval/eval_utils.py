...
def compute_camcoord_metrics(batch, pelvis_idxs=[1, 2], fps=30, mask=None):
    ...
    # ankle rotation error:
    pred_incam_full_body_pose = batch["pred_incam_pose"].cpu()
    gt_incam_full_body_pose = batch["gt_incam_pose"].cpu()
    # compute global instead of parent-relative joint rotation matrices:
    joint_indices = [7, 8]  # indices of ankles
    pred_global_rotations = batch_transfer_multi_rotation(
        batch["parents"],
        pred_incam_full_body_pose,  # inclusive global orient
        joint_indices=joint_indices,  # joint index of r_wrist
        transfer_type="l2g",  # local to global
        result_format="rotmat",
    )
    gt_global_rotations = batch_transfer_multi_rotation(
        batch["parents"],
        gt_incam_full_body_pose,  # inclusive global orient
        joint_indices=joint_indices,  # joint index of r_wrist
        transfer_type="l2g",  # local to global
        result_format="rotmat",
    )
    ...
    camcoord_metrics = {
        "pa_mpjpe": pa_mpjpe,
        "n_mpjpef": n_mpjpef * m2mm,  # foot mpjpe after per foot centering & scale
...
def joint_angle_error(r1, r2):
    ...
    :param pred_mat: predicted rotation matrices. Shape: ( ..., 3, 3)
    :param gt_mat: ground truth rotation matrices. Shape: ( ..., 3, 3)
    :return: Mean geodesic distance between input matrices.
    """
    ndims = r1.ndim
    # Transpose gt matrices (last two dims)
...
def joint_angle_error2(pred_mat, gt_mat):
    ...
    """
    Compute the geodesic distance between the two input matrices.
    :param pred_mat: predicted rotation matrices. Shape: ( Seq, J, 3, 3)
    :param gt_mat: ground truth rotation matrices. Shape: ( Seq, J, 3, 3)
    :return: Mean geodesic distance between input matrices.
    """

    # rotmat to quaternions:
...
def n_mpjpe(pred, gt, return_mean=True):
    ...
    (Does not minimize the mpjpe!)
    "Apply a single scale factor to the predic-
    tion so as to minimize the squared distance SE between la-
    bel and prediction"
